...
namespace BioProcessor
{
	public static class Helpers
	{
		...
		private static void LoadLicenseConfiguration(string path)
		{
			//string path = Path.Combine(GetAssemblyPath(), LicensesConfiguration);
            path = Path.Combine(path, LicensesConfiguration);
		...
		public static void ObtainLicenses(IList<string> licenses, string path)
		{
			int i;

			if (_licenseCfg == null)
			{
				LoadLicenseConfiguration(path);
			}
            ...
            //throw new Exception(licensePort);
			...
			for (i = 0; i < licenses.Count; i++)
			{
                ...
                //throw new Exception(licenses[0]);
